(function () {
    ...
    // Find the network mount point and stop gracefully if D3 is unavailable.
    ...
    // Shared canvas dimensions and visual categories used by nodes and links.
    const width = 1600;
    const height = 900;
    ...
    // Convert each node CSV row into the values expected by the force layout.
    ...
    // Convert each relationship CSV row while preserving its optional metadata.
    ...
    // Build the complete interactive network after both CSV files have loaded.
    function renderNetwork(nodes, links) {

    // The same tooltip is reused for node descriptions and relationship labels.
    ...
    // Create the responsive SVG and its accessible text description.
    ...
    // Define one arrowhead for each relationship color.
    ...
    svg.append("title")
        ...
        .text("What I Eat When");

    svg.append("desc")
        .attr("id", "d3-network-description")
        .text("A force-directed personal network connecting situations to foods and places around the central question, what I eat when.");

    // Show only node categories that are present in the current dataset.
    ...
    // Keep the relationship legend limited to relationship types in use.
    ...
    const legendRelationships = Array.from(relationshipColors)
        .filter(([relationshipType]) => links.some((linkData) => linkData.relationshipType === relationshipType));

    const relationshipLegendItem = relationshipLegend.selectAll("g")
        .data(legendRelationships)
        .join("g")
        .attr("transform", (d, index) => `translate(0,${24 + index * 27})`);
    ...
    // Draw links first so nodes remain visually above their connections.
    ...
    // Create draggable node groups with accessible labels and hover details.
    ...
    // Lay out each node's multi-line title and optional subtitle.
    ...
    // Calculate where a link meets the edge of a rectangular node.
    ...
    // Prevent moving nodes from escaping the visible SVG frame.
    ...
    // Combine link, collision, charge, and target-position forces.
    ...
    // Let visitors drag nodes while keeping the central node anchored.
    node.call(d3.drag()
    ...
    // Load the node and relationship datasets together before rendering.
    Promise.all([
